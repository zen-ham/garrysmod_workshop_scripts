...
def upload_gma(gmpublish_path, gma_path, icon_path):

    def ossystem(command):
        print(f"{'-'*80}\nRunning command: {command}\n{'-'*80}")
        os.system(command)

    command = [f'{gmpublish_path}', 'create', '-addon', f'"{gma_path}"', '-icon', f'"{icon_path}"']
    ossystem(' '.join(command))

from config import gmadexe, bsp, thumb, nav, map_name, tags, gmpublish

gma_path, icon_path = create_gma(gmadexe, bsp, thumb, map_name, tags)

upload_gma(gmpublish, gma_path, icon_path)