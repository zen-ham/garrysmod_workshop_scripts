...
from config import gmadexe, bsp, thumb, nav, map_name, tags, addon_id, gmpublish, changes

path_to_gma, icon_path = create_gma(gmadexe, bsp, thumb, map_name, tags)
...
if changes == '':
    command.append('"Small fix"')
else:
    command.append(f'"{changes}"')

if icon != '':
    command.extend(['-icon', f'"{icon}"'])
if path_to_gma != '':
    command.extend(['-addon', f'"{path_to_gma}"'])
